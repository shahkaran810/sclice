...
class SchemaController extends Controller
{
    ...
    // // Get custom plans created by the user
    // $user = Auth::user();
    // $userCustomPlans = $user->schemas()->where('status', true)->with('schedule')->get();

    // return view('frontend.schema.index', compact('schemas', 'userCustomPlans'));

        // Fetch all schemas with their schedules
    $schemas = Schema::where('status', true)
    ->with('schedule')
    ->get();

// Fetch custom plans created by the authenticated user
$user = Auth::user();
$userCustomPlans = CustomPlan::where('user_id', $user->id)->get();
