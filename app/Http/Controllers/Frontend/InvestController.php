...
{
    // dd("hello");
    return view('frontend.user.custom_plan');
}

public function submitCustomPlan(Request $request)
    {
        ...
        notify()->success('Custom plan created successfully!', 'success');
    ...
    public function investLogs(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('next_profit_time', function ($invest) {
                    return $invest->calculateNextProfitTime() ? $invest->calculateNextProfitTime()->format('Y-m-d') : 'N/A';
                })
...
//             ->addColumn('profit', 'frontend.user.include.__invest_profit')
//             ->addColumn('period_remaining', function ($raw) {
//                 return $raw->return_type != 'period' ? 'Unlimited' : $raw->number_of_period . ' Times';
//             })
//             ->editColumn('capital_back', 'frontend.user.include.__invest_capital_back')
//             ->editColumn('next_profit_time', function ($invest) {
//                 return $invest->next_profit_time ? $invest->next_profit_time->format('Y-m-d') : 'N/A';
//             })
//             ->rawColumns(['icon', 'schema', 'rio', 'profit', 'capital_back', 'next_profit_time'])
//             ->make(true);
//     }
